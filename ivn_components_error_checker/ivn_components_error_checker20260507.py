...
def resolve_duplicates(df, col, threshold=95, prompt_message="Choose the correct value:"):
...
def prompt_user_choice(options, prompt_message):
    """Prompt user to choose how to resolve a set of similar options.
    Options:
      - Enter 'A' to accept all as validly unique (no renaming)
      - Enter numbers separated by commas to select a subset to rename, then prompt for the new name
      - Enter a single number to pick one as canonical (legacy behavior)
    Returns:
      - None if all are validly unique
      - (indices, new_name) if a subset is to be renamed
      - canonical value if a single value is chosen
    """
    print(prompt_message)
    for i, option in enumerate(options, 1):
        print(f"{i}: {option}")
    print("A: All are validly unique (no renaming)")
    while True:
        user_input = input("Enter 'A' for all unique, a single number to assign that name to all, or multiple numbers (e.g. 1,2) to select a subset to rename: ").strip()
        if user_input.upper() == 'A':
            return None  # All are validly unique
        nums = [n.strip() for n in user_input.split(',') if n.strip()]
        if all(n.isdigit() and 1 <= int(n) <= len(options) for n in nums):
            if len(nums) == 1:
                # Assign that name to all similar components
                canonical = options[int(nums[0]) - 1]
                return ('all', canonical)
            else:
                # Prompt user to pick which name to assign to all selected
                subset_indices = [int(n) - 1 for n in nums]
                print("Choose the name to assign to all selected components:")
                for idx in subset_indices:
                    print(f"{idx+1}: {options[idx]}")
                while True:
                    name_choice = input(f"Enter the number (from above) to use as the new name for all selected: ").strip()
                    if name_choice.isdigit() and int(name_choice)-1 in subset_indices:
                        new_name = options[int(name_choice)-1]
                        return (subset_indices, new_name)
...
def resolve_duplicates(df, col, group_col, threshold=95, prompt_message="Choose the correct value:"):
    ...
    for indices in to_process:
        original_values = df.loc[indices, col].dropna().unique()
        if len(original_values) > 1:
            result = prompt_user_choice(list(original_values), f"\n{prompt_message}\nOptions for similar '{col}':")
            if result is None:
                # User chose 'A' (all unique), do nothing
                continue
            elif isinstance(result, tuple):
                subset_indices, new_name = result
                values = list(original_values)
                if subset_indices == 'all':
                    # Assign canonical name to all similar components
                    df.loc[indices, col] = new_name
                else:
                    # User selected subset to rename
                    for idx in subset_indices:
                        val = values[idx]
                        # Find all rows with this value in the indices group
                        mask = df.index[indices][df.loc[indices, col] == val]
                        df.loc[mask, col] = new_name
            else:
                # User picked a canonical value (should not occur with new logic, but kept for safety)
                df.loc[indices, col] = result
...
def main():
    ...
    # List all sheet names and prompt user to select one
    ...
    print(f"FGDEC: Columns in selected sheet '{selected_sheet}':")
    ...
    print(f"FGDEC: You selected column '{selected_column}' in sheet '{selected_sheet}'.")

    # Step 1: Show non-unique values (exact)
    value_counts = df[selected_column].value_counts(dropna=False)
    non_unique_values = value_counts[value_counts > 1]
    if non_unique_values.empty:
        print(f"FGDEC: All values in column '{selected_column}' are unique.")
    else:
        print(f"FGDEC: The following values in column '{selected_column}' are NOT unique (appear more than once):")
        for val, count in non_unique_values.items():
            print(f"  Value: {repr(val)} | Count: {count}")
        show_rows = input("Would you like to see the rows with these non-unique values? (y/n): ").strip().lower()
        if show_rows == 'y':
            for val in non_unique_values.index:
                print(f"\nRows for value: {repr(val)}")
                print(df[df[selected_column] == val])

    # Step 2: Fuzzy/normalized duplicate detection and interactive resolution
    print(f"\nFGDEC: Checking for possible false unique values (fuzzy/normalized duplicates) in '{selected_column}'...")
    df = resolve_duplicates(df, selected_column, threshold=90, prompt_message="Choose the canonical value for these similar entries")
    print(f"FGDEC: Fuzzy/normalized duplicate resolution complete.")
