"""Write to a NEW file with different name."""
content = '''# filepath: ivn_pipeline_v2.py
"""
...
class IVNProductionPipeline:
    ...
    def _build_lookups(self):
        ...
        for _, row in self.sources_df.iterrows():
            src_id = self._safe_get(row, SOURCES_COLS['id'])
            if src_id:
                self._source_lookup[src_id] = self._safe_get(row, SOURCES_COLS['name'])
                self._agency_lookup[src_id] = self._safe_get(row, SOURCES_COLS['agency'])
    ...
    def _is_same_source(self, a, b):
        ...
        a_base, b_base = a_norm, b_norm
        for s in suffixes:
            a_base = a_base.replace(s, '').strip()
            b_base = b_base.replace(s, '').strip()
        return a_base and b_base and a_base == b_base
    ...
    def generate_and_score_candidates(self):
        ...
        candidates = []
        rejected_same, rejected_self, rejected_low = 0, 0, 0
        n_tbc = len(self.to_be_crosswalked_df)
        n_comp = len(self.components_df)
        logger.info(f"Comparing {n_tbc} ToBeCrosswalked vs {n_comp} Components, min_score={min_score}")
        
        for idx, tbc in self.to_be_crosswalked_df.iterrows():
            if idx % 100 == 0:
                logger.info(f"Processing row {idx}/{n_tbc}")
        ...
        logger.info(f"Generated {len(self.candidates_df):,} pairs")
        logger.info(f"Rejected same-source: {rejected_same:,}, self-match: {rejected_self:,}, low-score: {rejected_low:,}")
...
if __name__ == '__main__':
    ...
    print(f"Output: {len(result):,} pairs saved to {args.output}")
'''

# Write to a NEW filename
with open('ivn_pipeline_v2.py', 'w', encoding='utf-8') as f:
    f.write(content)

import os
print(f"Wrote {os.path.getsize('ivn_pipeline_v2.py')} bytes to ivn_pipeline_v2.py")
